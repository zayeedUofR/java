...
public class HistogramEQ {
    ...
    private static BufferedImage histogramEqualization(BufferedImage original) {
        ...
        for(int i=0; i<original.getWidth(); i++) {
            for(int j=0; j<original.getHeight(); j++) {
                ...
                // Set new pixel values using the histogram lookup table, geting the corresponding equilized value of RGB 
                //corresponding to the origimal RGB
        ...
        return histogramEQ; // final image
 
    }
 
    // Get the histogram equalization lookup table for separate R, G, B channels
    private static ArrayList<int[]> histogramEqualizationLUT(BufferedImage input) { //cumulative histogram
    ...
    public static ArrayList<int[]> imageHistogram(BufferedImage input) {     // histogram
    ...
    //HistogramEQ ob = new HistogramEQ();
    
    BufferedImage outImg;
        outImg = HistogramEQ.computeHistogramEQ(image);
    
    File outputfile = new File("RGB_equlilized_prb_1_ouput.jpg");
    ImageIO.write(outImg, "jpg", outputfile);
    
    System.out.println("Hisstoram Equlization was applied on R, G and B channels seperately and the New image is saved as RGB_equlilized_prb_1_ouput.jpg\nat application base directory.");
